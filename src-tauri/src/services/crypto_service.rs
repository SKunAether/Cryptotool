...
impl CryptoService {
    ...
    pub fn encrypt(key_b64: &str, plaintext: &str) -> Result<String, AppError> {
        ...
        crypto_engine::encrypt_aes256_gcm(&key_bytes, plaintext).map_err(|e| {
            AppError::with_params("crypto.encrypt_failed", vec![e], ErrorSeverity::Error)
        })
    }
    ...
    pub fn decrypt(key_b64: &str, ciphertext_b64: &str) -> Result<String, AppError> {
        let key_bytes = decode_key(key_b64)?;
        crypto_engine::decrypt_aes256_gcm(&key_bytes, ciphertext_b64).map_err(|e| {
            AppError::with_params("crypto.decrypt_failed", vec![e], ErrorSeverity::Error)
        })
    }
}
